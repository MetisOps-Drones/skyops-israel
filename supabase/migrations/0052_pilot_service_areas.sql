-- Design-partner audit (map/marketplace round): a pilot's geography was the
-- single most obviously-missing filter dimension for a physically-bound
-- service like drone piloting — an org had no way to filter/see "works in
-- the Sharon / Negev / nationwide" before hiring. Adds a tagged service-area
-- list to the marketplace pilot profile, alongside the existing skill/drone
-- taxonomy, and surfaces it through both marketplace RPCs.

alter table pilot_profiles
  add column if not exists service_areas text[] not null default '{}';

-- Return shape is changing again, so drop before recreate as in 0051.
