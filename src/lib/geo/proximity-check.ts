 ...
 * guarantee.
 *
 * Used both by the client-facing advisory route
 * (src/app/api/proximity-check/route.ts) and by the server-side
 * authoritative check in actions/flight-requests.ts — extracted here
 * because a Next.js route.ts file may only export the reserved
 * HTTP-method handlers, not arbitrary helper functions.
 */
...
export async function checkProximity(lat: number, lon: number): Promise<ProximityCheckResult> {
  ...
  if (cached && Date.now() - new Date(cached.checked_at).getTime() < CACHE_MAX_AGE_MS) {
    return { available: true, findings: cached.findings as unknown as ProximityFinding[], cached: true };
  }
  ...
  try {
    ...
    if (cached) return { available: true, findings: cached.findings as unknown as ProximityFinding[], cached: true, stale: true };
  ...
  return { available: true, findings, cached: false };
}
